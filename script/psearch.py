################################################################################
#
# python file for passing series of parameters to the submit.py HTCondor script
# then automatically call submiy.py to submit jobs to the Open Science Grid
    ...
    for k in range (0, len(rlow)):
      for l in range (0, len(m_rate)):
        ...
        call(['python3', '../src/submit.py',
              str(dump_in), str(img_in), str(rlow_in), 
              str(rhigh_in), str(munit_in), str(theta_in)])
